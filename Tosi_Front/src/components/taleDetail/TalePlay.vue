<template>
  <div v-if="taleDetailStore.pages">
    <div class="play">
      ...
      <img v-if="isPaused" src="@/assets/playaudio.png" @click="audioPause" class="audio" />
      <img v-else src="@/assets/pause.png" @click="audioPause" class="audio" />
    </div>
  </div>
...
function flipPage(index, flip) {
  ...
  if (flip) {
    ...
    currentPageNum.value = pages.length - currentPageIndex.value; // 현재 페이지 번호 업데이트
  } else {
